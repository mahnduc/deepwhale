      ...
      <div className="max-w-7xl mx-auto">
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-10">
            {agents.map((agent) => (
              <div key={agent.agent_id} className={`group rounded-[2.5rem] bg-base-200 p-6 flex flex-col items-center transition-all duration-300 ${neoFlat} hover:scale-[1.02]`}>
                <div className={`w-32 h-32 rounded-full bg-base-200 flex items-center justify-center p-3 mb-6 ${neoInset}`}>
                  <div className="w-full h-full rounded-full overflow-hidden bg-base-300/20">
                    <img src={agent.avatarUrl} alt={agent.name} className="w-full h-full object-contain transition-transform group-hover:scale-110" />
                  </div>
                </div>
                <div className="text-center w-full mb-6">
                  <h2 className="text-xl font-black tracking-tight truncate">{agent.name}</h2>
                ...
                <p className="text-xs text-center opacity-60 h-12 overflow-hidden line-clamp-2 mb-8 italic px-2">"{agent.description}"</p>
                <Link 
                ...
                <div className="mt-6 text-[8px] font-bold opacity-20 uppercase tracking-tighter">
                  ID: {agent.agent_id.split('-')[0]} | {new Date(agent.createdAt).toLocaleDateString()}
                </div>
              </div>
            ))}
          </div>

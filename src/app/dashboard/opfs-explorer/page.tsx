...
export default function OPFSExplorer() {
  ...
  const handleCreate = async (type: NodeType) => {
    const name = prompt(
      `Nhập tên ${type === "folder" ? "thư mục" : "tập tin"}:`
    );
    ...
    try {
      if (type === "folder") {
        await opfsApi.createFolder(fullPath);
      } else {
        await opfsApi.createEmptyFile(fullPath);
      }

      if (!expanded.includes(currentPath)) {
        setExpanded((prev) => [...prev, currentPath]);
      }

      await loadTree();
    } catch (err) {
      alert("Lỗi tạo mới: " + err);
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (editingFile?.path === selectedNode.path) {
        setEditingFile(null);
      }
            ...
            className={`flex items-center gap-2 px-3 py-1.5 cursor-pointer transition-all duration-200 group rounded-lg mb-0.5
              ${
                isSelected
                  ...
                  : "hover:bg-[#F7F9FB] text-[#2D3436] font-bold"
              }`}
          >
            {node.type === "folder" ? (
              <Folder
              ...
              <FileText
                size={16}
                className={
                  isSelected ? "text-[#FF3399]" : "text-[#B2BEC3]"
                }
              />
            )}
    ...
    <div className="flex h-full w-full overflow-hidden bg-white">
      ...
      <main className="flex-1 flex flex-col bg-white min-w-0">
        {editingFile ? (
          <div className="flex flex-col h-full animate-in fade-in duration-200">
            <div className="h-14 px-6 flex items-center justify-between border-b border-[#F0F0F0]">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[13px] text-[#2D3436] font-black uppercase tracking-tight">
                  {editingFile.name}
                </span>
              </div>
              ...
              <button
                ...
                className="p-1.5 hover:bg-[#F7F9FB] rounded-md text-[#B2BEC3] hover:text-[#FF3399] transition-all"
              >
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-[#B2BEC3] bg-[#FDFDFD]">
            ...
            <p className="font-bold text-[13px] uppercase tracking-widest opacity-40">
              Chọn tệp để chỉnh sửa
            </p>
          </div>
        )}
      </main>
      ...
      <aside className="w-16 flex flex-col items-center py-6 gap-3 bg-[#F7F9FB] border-l border-[#F0F0F0] shrink-0">
        <button
          ...
          className={`p-3 rounded-xl border-b-2 active:translate-y-0.5 active:border-b-0 transition-all ${
            editingFile
              ? "bg-[#00CEC9] text-white border-[#00A8A5] shadow-md"
              : "bg-white text-[#E5E5E5] border-[#E5E5E5]"
          }`}
        >
        ...
        <div className="mt-auto">
          <button
            ...
            className="p-3 text-[#B2BEC3] hover:text-[#FF3399] hover:bg-[#FFF0F7] rounded-xl transition-all"
          >
...
const ToolbarButton = ({ icon, onClick, loading, color }: any) => (
  <button
    onClick={onClick}
    className={`p-3 bg-white rounded-xl border border-[#E5E5E5] border-b-[3px] hover:border-[#FF3399]/30 active:translate-y-0.5 active:border-b-px transition-all ${
